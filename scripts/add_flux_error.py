...
import cPickle
import os
from spec_id import Stack_spec_normwmean,Analyze_Stack_avgage,Likelihood_contours,Identify_stack, Analyze_Stack, Scale_model
...
def Stack_spec_normwmean_eog(spec,redshifts, wv):
...
def Stack_spec_normwmean_eos(spec,redshifts, wv):
    ...
    return wv[IDX], stack[IDX], np.sqrt(err[IDX] ** 2 + (stack[IDX] * .1) ** 2)

def Stack_model_normwmean_eos(speclist, modellist, redshifts, redshiftbins, wv_range):
...
def Model_fit_stack_normwmean_eos(speclist, tau, metal, A, speczs, wv_range,name, pkl_name, res=5, fsps=False):
    ...
    wv,fl,err=Stack_spec_normwmean_eos(speclist,speczs,wv_range)
    ...
    if os.path.isfile(pklname)==False:
        ...
        for i in range(len(metal)):
            for ii in range(len(A)):
                for iii in range(len(tau)):
                    mw, mf, me = Stack_model_normwmean_eos(speclist,modellist[i][ii][iii], speczs,
                                                           zlist, np.arange(wv[0],wv[-1]+res,res))
...
#
wv,fl,er=Stack_spec_normwmean_eog(speclist,rshift[IDS],np.arange(3250,5500,5))
wv5,fl5,er5=Stack_spec_normwmean_eos(speclist,rshift[IDS],np.arange(3250,5500,5))
wvnm,flnm,ernm=Stack_spec_normwmean(speclist,rshift[IDS],np.arange(3250,5500,5))
ftwv,fts,fte=Stack_model_normwmean_eos(speclist,flist, speczs, zlist, np.arange(wv[0],wv[-1]+5,5))


plt.plot(wv,fl,color='#6e7da2',label='add before stacking')
plt.plot(wvnm,flnm,color='k',label='no error added')
plt.plot(wv5,fl5,color='#a13535',label='add after stacking')
plt.plot(wv,er,color='#6e7da2')
plt.plot(wvnm,ernm,color='k')
plt.plot(wv5,er5,color='#a13535')
# plt.plot(ftwv,fts)
...
plt.legend(loc=2)
# plt.show()
plt.savefig('../research_plots/error_add_comp.png')
plt.close()
...
#
# Model_fit_stack_normwmean_eos(speclist,tau,metal,age,rshift[IDS],np.arange(3250,5500,5),
#                               'gt10.87_eos_stackfit20','gt10.87_fsps_nwmeannm_spec',fsps=True)
#
# Pr,bfage,bfmetal=Analyze_Stack_avgage('chidat/gt10.87_eos_stackfit20_chidata.fits', np.array(tau),metal,age)
...
# plt.legend()
# # plt.show()
# plt.savefig('../research_plots/gt10.87_eos_stackfit20_likelihood.png')
# plt.close()