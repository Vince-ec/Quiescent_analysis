...
def MC_fit_methods_test(galaxy, metal, age, tau, sim_m, sim_a, sim_t, specz, minwv=7900, maxwv=11400, repeats=1,
           age_conv='../data/tau_scale_ntau.dat'):
    ...
    for xx in range(repeats):
        ...
        # MIN = np.argwhere(newchi == np.min(newchi))
        # print metal[MIN[0][0]],age[MIN[0][1]],tau[MIN[0][2]]
        # MIN = np.argwhere(newNCchi == np.min(newNCchi))
        # print metal[MIN[0][0]],age[MIN[0][1]],tau[MIN[0][2]]
        # MIN = np.argwhere(comb_prob == np.max(comb_prob))
        # print metal[MIN[0][0]],age[MIN[0][1]],tau[MIN[0][2]]
        #
        plt.figure()
        plt.subplot(131)
        plt.contourf(np.log(M),np.log(A),np.log(P.T/C))
        plt.scatter(np.log(sim_m),np.log(sim_a))
        plt.axis(np.log([0.002,0.03,0.5,6]))

        plt.subplot(132)
        plt.contourf(M,A,Pnc.T/Cnc)
        plt.scatter(sim_m,sim_a)
        plt.axis([0,0.03,0,6])

        plt.subplot(133)
        plt.contourf(M,A,df_post.T)
        plt.scatter(sim_m,sim_a)
        plt.axis([0,0.03,0,6])
